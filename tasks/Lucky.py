...
def lucky(num):
    if type(num) is not int:
        ...
        for i in range(num // 2 - 1):
            ...
            #Каждый элемент вычисляется путем суммирования всех N(k) до данной позиции включительно используя массив созданный на предыдущей итерации, но не более 9-ти
            #Для перехода от Массива для единичных билетов к дву- трех- значным используем такой подход:
            #Если предположить что номер билета начинается с числа j То Суммма номеров билета является k-j, где j принимает значения 0..9 и не превышает k.
            #Соответственно количество номеров сумм выражается формулой N(k-j), где j = 0..9.
